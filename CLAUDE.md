...
python3 scripts/recommend_new.py  # 弱点トピックと推奨難易度から今日の1問を表示
```

### 新問題の日
```bash
python3 scripts/recommend_new.py          # 今日の1問を見る（弱点トピック・難易度を自動判断）
python3 scripts/new_problem.py <番号>        # 問題追加
...
- `recommend_new.py` が弱点トピックと推奨難易度から今日解く未登録の1問を決定
  - 自動判定は基本 Easy / Medium。Medium習得量・低リトライ率・直近安定の条件が揃えば Hard も表示
